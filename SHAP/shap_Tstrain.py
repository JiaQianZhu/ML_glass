...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to generate the shap plots of Tstrain.
"""
...
column_index_Tstrain = list(y_val.columns).index('Tstrain')
shap_values_Tstrain = shap_values[column_index_Tstrain]


# Plot the SHAP summary plot for 'Tstrain'.
shap.summary_plot(shap_values_Tstrain, X_val, feature_names=X_val.columns, show=False)
plt.title("SHAP on Tstrain", fontweight='bold', fontsize=18)
plt.subplots_adjust(top=0.9)
plt.xticks(fontweight='bold')
plt.yticks(fontweight='bold')  
plt.savefig("../results/shap/Tstrain.jpg", dpi=300)
