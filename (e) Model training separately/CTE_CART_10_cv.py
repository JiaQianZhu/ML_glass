...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to filter out the non-missing portions of CTE and conduct training using the CART algorithm with a 10-fold cv.
"""
...
non_empty_df = data.dropna(subset=['CTEbelowTg'])

# Define input features and target variable
features = non_empty_df[['SiO2', 'Al2O3', 'CaO', 'MgO', 'SrO', 'B2O3', 'Sb2O3']]
target = non_empty_df['CTEbelowTg']
...
for train_idx, val_idx in kf.split(train_features):
    ...
    cart_model.fit(train_fold_features, train_fold_target)

    # Make predictions
    val_predictions = cart_model.predict(val_fold_features)
...
test_predictions = cart_model.predict(test_features)
...
with open('cart_model.pickle', 'wb') as file:
