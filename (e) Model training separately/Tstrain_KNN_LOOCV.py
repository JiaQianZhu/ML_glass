...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to filter out the non-missing portions of Tstrain and conduct training using the KNN algorithm with LOOCV.
"""
import pickle
import pandas as pd
...
non_empty_df = data.dropna(subset=['Tstrain'])
...
for index, row in non_empty_df.iterrows():
    ...
    train_target = non_empty_df.drop(index)['Tstrain']
...
with open('../model/individual_prediction/Tstrain_KNN_model.pickle', 'wb') as file:
