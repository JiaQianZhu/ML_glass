...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to filter out the non-missing portions of CTE and conduct training using the KNN algorithm with a 10-fold cv.
"""
import pickle
import numpy as np
import pandas as pd
...
# Create a K-Nearest Neighbors regression model (KNN)
...
for train_idx, val_idx in kf.split(train_features):
    ...
    knn_model.fit(train_fold_features, train_fold_target)

    # Make predictions
    val_predictions = knn_model.predict(val_fold_features)
