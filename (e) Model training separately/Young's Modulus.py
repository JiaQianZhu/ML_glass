...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to filter out the non-missing portions of Young's Modulus and proceed with training.
"""
...
non_empty_df = data.dropna(subset=['YoungModulus'])
...
for index, row in non_empty_df.iterrows():
    ...
    train_target = non_empty_df.drop(index)['YoungModulus']

    # Create KNN regression model.
    knn_model = KNeighborsRegressor(n_neighbors=5)  # 可根据需要调整 n_neighbors 参数
...
with open('../model/individual_prediction/Young_knn_model.pickle', 'wb') as file:
