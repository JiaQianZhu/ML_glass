...
Author:JiaQian Zhu
Date:2023-8-17
Usage:This module offers method to generate the scatter density plot of CTE.
"""
...
scatter = ax.scatter(y_test, y_test_pred, c=y_test - y_test_pred, cmap='cool',
                     s=3000000 * (np.abs(y_test - y_test_pred)))

# Add a colorbar.
cbar = plt.colorbar(scatter, ax=ax)
cbar.set_label('CTE', weight='bold', fontsize=16)
cbar.ax.get_yaxis().labelpad = 15
cbar.ax.tick_params(labelsize=14)
cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontweight='bold')

# Restore scientific notation format for colorbar tick labels.
formatter = ScalarFormatter(useMathText=True)
formatter.set_powerlimits((0, 0))
cbar.ax.yaxis.set_major_formatter(formatter)

# Add a unit above the colorbar.
cbar.ax.annotate('1e-6', xy=(0.5, 1.03), xycoords='axes fraction', ha='center', fontweight='bold', fontsize=14)

# Set bold font for colorbar tick labels.
cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontweight='bold')
...
# Set the axis labels.
ax.set_ylabel('Predicted Values of CTE', fontsize=16, fontweight='bold')
...
# Set ScalarFormatter for tick labels and apply bold font.
formatter = ScalarFormatter(useMathText=True)
formatter.set_powerlimits((0, 0))
ax.xaxis.set_major_formatter(formatter)
ax.yaxis.set_major_formatter(formatter)

# Set bold font for offset text.
...
ax_inset1.hist(y_test - y_test_pred, bins=550, color='blue', alpha=0.8)
ax_inset1.axvline(0, color='black', ls='--')
ax_inset1.set_xlabel('Pred.residual', labelpad=15, fontsize=13, fontweight='bold')
...
ax_inset1.set_xlim(-0.000001, 0.000001)
...
ax_inset1.xaxis.set_major_formatter(formatter)

# Set the font weight for the offset text (scientific notation) in the inset subplot
ax_inset1.xaxis.offsetText.set_fontweight('bold')

# Set the y-axis range to 0 to 300
ax_inset1.set_ylim(0, 300)
...
ax_inset2 = fig.add_axes([0.41, 0.25, 0.25, 0.15], facecolor='blue', frame_on=True)
ax_inset2.axis('off')
ax_inset2.text(0, 0.8, f'R² = {r2:.4f}', fontsize=18, fontweight='bold')
ax_inset2.text(0, 0.6, f'RMSE = {rmse:.2e}', fontsize=18, fontweight='bold')
ax_inset2.text(0, 0.4, f'MAE = {mae:.2e}', fontsize=18, fontweight='bold')
ax_inset2.text(0, 0.2, f'MedAE = {medae:.2e}', fontsize=18, fontweight='bold')
